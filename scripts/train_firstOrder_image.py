...
def create_dataset(nb_categories, nb_exemplars, save_folder):
    ...
    if os.path.isdir(save_folder):
        warnings.warn('Removing existing folder at %s' % save_folder)
    ...
    for shape, color, texture, i in \
        ...
        save_file = os.path.join(save_folder, 'img%0.4i.png' % i)
...
def load_dataset(save_folder):
    ...
    feature_file = os.path.join(save_folder, 'data.csv')
...
def main():
    ...
    """
    create_dataset(args.nb_categories, args.nb_exemplars, args.save_folder)
    X, shapes = load_dataset(args.save_folder)
...
if __name__ == '__main__':
    ...
    parser.set_defaults(save_folder='../data/temp')
